...
def record_or_load_spectrum_without_fiber(spectrometer, data_directory, time_interval_seconds, integration_time_ms, time_background, avg_background):
    ...
    try:
        spectrometer_name = spectrometer.model.replace(" ", "_") if spectrometer else "UnknownSpectrometer"
        spectrum_without_fiber_filename, spectrum_without_fiber_averaged_filename = generate_filenames_without_fiber(data_directory, integration_time_ms, spectrometer_name)
...
def generate_filenames_without_fiber(data_directory, integration_time_ms, spectrometer_name):
    spectrometer_name = spectrometer_name.replace(" ", "_")  # Replace spaces with underscores
    filename = os.path.join(data_directory, f"averaged_spectrum_without_fiber_{spectrometer_name}_{integration_time_ms}ms.h5")
    averaged_filename = os.path.join(data_directory, f"averaged_spectrum_without_fiber_{spectrometer_name}_{integration_time_ms}ms.h5")
...
def normalize_power():
    power_measurement_wavelength = 650
    ...
    return input_power_uW, background_input_power_uW, output_power_uW, background_output_power_uW, power_percentage

def save_power_settings(data_directory, input_power_uW, background_input_power_uW, output_power_uW, background_output_power_uW, wavelength_nm):
    current_date = time.strftime('%Y%m%d')
    base_filename = f"power_settings_{wavelength_nm}nm_{current_date}"
    counter = 1
    power_settings_filename = os.path.join(data_directory, f"{base_filename}.txt")
    while os.path.exists(power_settings_filename):
        power_settings_filename = os.path.join(data_directory, f"{base_filename}({counter}).txt")
        counter += 1
    with open(power_settings_filename, "w") as file:
        file.write(f"Input Power (μW): {input_power_uW}\n")
        file.write(f"Background Input Power (μW): {background_input_power_uW}\n")
        file.write(f"Output Power (μW): {output_power_uW}\n")
        file.write(f"Background Output Power (μW): {background_output_power_uW}\n")

    print("Power settings saved to:", power_settings_filename)
...
def save_data_to_csv(csv_path, data_dict):
    with open(csv_path, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        for key, value in data_dict.items():
            writer.writerow([key] + list(value))
    print(f"Saved data to CSV file: {csv_path}")

def convert_h5_to_csv(input_h5_path, output_csv_path):
    with h5py.File(input_h5_path, 'r') as file:
        data_dict = {key: file[key][:] for key in file.keys()}
    save_data_to_csv(output_csv_path, data_dict)

def convert_and_save_all_to_csv(h5_folder, csv_folder):
    for root, dirs, files in os.walk(h5_folder):
        for file in files:
            if file.endswith(".h5"):
                h5_path = os.path.join(root, file)
                csv_path = os.path.join(csv_folder, file.replace(".h5", ".csv"))
                convert_h5_to_csv(h5_path, csv_path)
    print(f"All HDF5 files in {h5_folder} converted and saved as CSV files in {csv_folder}")
...
def main():
    try:
        ...
        input_power_uW, background_input_power_uW, output_power_uW, background_output_power_uW, power_percentage = normalize_power()
        save_power_settings(data_directory, input_power_uW, background_input_power_uW, output_power_uW, background_output_power_uW)
        ...
        convert_and_save_all_to_csv("./.h5_files", csv_folder)
