...
def get_spectra_measurement_settings():
    ...
    spectra_number_of_spectra = 3
    spectra_integration_time_ms = 20
...
def record_spectra(spectrometer, spectra_time_interval_seconds, spectra_integration_time_ms, spectra_total_duration_seconds):
    spectra = []
    timestamps = []
    ...
    while (time.time() - start_time) <= spectra_total_duration_seconds:
        spectrum_data = spectrometer.spectrum(correct_dark_counts=True)
        wavelengths = spectrometer.wavelengths()
        spectra.append((wavelengths, spectrum_data))
        timestamps.append(time.time())
...
def save_individual_spectrum(filename, wavelengths, spectrum_data, timestamp):
    save_data_to_hdf5(filename, {
        "wavelengths": wavelengths,
        "spectrum_data": spectrum_data,
        "timestamp": timestamp
    })
    print(f"Individual spectrum saved to: {filename}")
...
def main():
    try:
        ...
        spectra, timestamps = record_spectra(spectrometer, spectra_time_interval_seconds, spectra_integration_time_ms, spectra_total_duration_seconds)

        print("Saving individual spectra and plotting...")
        for i, (wavelengths, spectrum_data) in enumerate(spectra):
            filename = os.path.join(data_directory, f"individual_spectrum_{spectra_integration_time_ms}ms_{i}.h5")
            save_individual_spectrum(filename, wavelengths, spectrum_data, timestamps[i])
